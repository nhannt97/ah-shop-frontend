/* @-<zoom ********************************************************************/
/******************************************************************************/
export default function zoom(classNames, settings) {
    /* Settings */
    ...
    let C_scaleDifference = settings["scaleDifference"] || 0.5; // Used on wheel zoom
    let C_scaleMax = settings["scaleMax"] || 10;
    ...
    let $container;
    let $element;
    let $zoom = document.querySelectorAll('div[type="zoom"]');
    ...
    for (let i = 0; i < $zoom.length; i++) {
        ...
        moveScaleElement($element, 0 + "px", 0 + "px", 1);
    }
...
function hasClass($element, targetClass) {
    let rgx = new RegExp("(?:^|\\s)" + targetClass + "(?!\\S)", "g");
...
function massAddEventListener($elements, event, customFunction, useCapture) {
    for (let i = 0; i < $elements.length; i++) {
        $elements[i].addEventListener(event, customFunction, useCapture || false);
    }
}
